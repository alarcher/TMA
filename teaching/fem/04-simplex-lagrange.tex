...
Lagrange polynomials \eqref{def:lagrange_poly} are used to build directly the one-dimensional shape functions, while in higher dimensions they can be reformulated in term of barycentric coordinates.

\begin{dfntn}[Barycentric coordinates]
\begin{eqnarray*}
\lambda_i:& \xR^d &\fromto \xR\\
          & \bfx  &\mapsto \lambda_i(\bfx) = 1 - \dfrac{(\bfx - \Pnode_i)\cdot\n_i}{(\Pnode_{f} - \Pnode_i)\cdot\n_i}
\end{eqnarray*}
with $\n_i$ the unit outward normal to the facet opposite to $\Pnode_i$, and $\Pnode_f$ a node belonging to this facet.
\end{dfntn}

The geometric interpretation of barycentric coordinates is given by
\[
\displaystyle\lambda_i = \frac{|K_i|}{|K|}
\]
with $K_i(\xx)$ the $d$-simplex constructed with point $\xx$ and the facet opposite to $\Pnode_i$, and in particular the point satisfying equal weight $\lambda_i = (d+1)^{-1}$ is the isobarycentre.
In practice this property can be used to check if a point is inside a simplex: if $\sum_i |K_i(\xx)| > |K|$ then the point is outside of the simplex.
...
\coordinate (hv1) at (2,0);
\coordinate (hv2) at (0,2);
...
with
\begin{equation*}
\matJ_{T_K} = \left[\frac{\partial T_{K,i}}{\partial x_j}  \right]_{ij}
\end{equation*}
...
= \sum_{j} \frac{\partial\varphi}{\partial \hat x_j} \frac{\partial T_{K,j}^{-1}}{\partial x_i}
\end{equation*}
