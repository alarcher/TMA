...
\begin{prpstn}[Interpolation Inequality for Lagrange $\xP_k$, \cite{EG}]
Let $0\leq s \leq k$, there exists a positive constant $C$ such that for any $v\in\xH^{s+1}(\dim)$
\begin{equation*}
\norm{v-\Lgrh{1}v}_{\xLtwo(K)} + \sizeT \snorm{v-\Lgrh{1}v}_{\xHone(K)}\leq C h^{s+1}_K\snorm{v}_{\xH^{s+1}(K)}
\end{equation*}
\end{prpstn}
