...
\displaystyle\int_\dom \Grad{u}\xDot\Grad{v} \dxx = \int_\dom f\;v \dxx\;,\quad\forany v \in\xV\\[2ex]
\end{array}
\right.
\end{equation*}
...
\displaystyle\int_\dom \Grad{u_h}\xDot\Grad{v} \dxx = \int_\dom f\;v \dxx\;,\quad\forany v \in\xVh\\[2ex]
\end{array}
\right.
\end{equation*}
...
\begin{equation*}
E = \snorm{u - u_h}^2_{\xHone(\dom)} = \int_\dom \Abs{\Grad(u - u_h)}^2\dxx = \int_\dom \Grad(u - u_h)\xDot\Grad(u - u_h) \dxx
\end{equation*}

For any $v_h\in\xVh$
\begin{equation*}
E = \int_\dom \Grad(u - u_h)\xDot\Grad(u - v_h) \dxx + \int_\dom \Grad(u - u_h)\xDot\Grad(v_h - u_h) \dxx
\end{equation*}
and the second term cancels by virtue of consistency (Galerkin orthogonality), so that Cauchy--Scwarz gives
\begin{equation*}
E = \snorm{u - u_h}^2_{\xHone(\dom)} \leq \left(\int_\dom \Abs{\Grad(u - u_h)}^2 \dxx\right)^{\sfrac{1}{2}} \left(\int_\dom \Abs{\Grad(u - v_h)}^2 \dxx\right)^{\sfrac{1}{2}}
\end{equation*}
thus
\begin{equation*}
...
\begin{equation*}
\begin{array}{lcl}
\norm{\Lgrh{1}v}_{\xLtwo(\dom)} &=&\displaystyle \left(\int_\dom \Abs{\Lgrh{1}v}^2 \dxx\right)^{\sfrac{1}{2}}\\[2ex]
...
\displaystyle\int_\dom \Grad \varphi\xDot \Grad \phi\dxx = \int_\dom e_h \phi  \dxx\quad,\;\forany  \phi\in \xHonec(\dom)
\end{array}
\right .
\end{equation}
...
\begin{equation*}
\nLtwo{e_h} = \int_\dom |e_h|^2 \dxx = \int_\dom \Grad \varphi\xDot \Grad e_h\dxx
\end{equation*}
...
\begin{equation*}
\int_\dom \Grad \varphi_h\xDot \Grad e_h\dxx  = 0
\end{equation*}
Thus we can subtract and add this latter to the previous expression:
\begin{equation*}
\nLtwo{e_h} = \int_\dom \Grad (\varphi - \varphi_h)\xDot \Grad e_h\dxx + \underbrace{\int_\dom \Grad \varphi_h\xDot \Grad e_h\dxx}_{0}
\end{equation*}
